...
import { Upload, Cpu, Zap, AlertCircle, Lock, Loader2 } from "lucide-react";
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === "done") {
          ...
          clearInterval(interval);
          setStatus("error");
          setError("Inference engine failure.");
        } else {
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="rounded-3xl border border-white bg-white/60 p-6 shadow-xl backdrop-blur-xl">
              ...
              <div className="space-y-3">
                {["convnext", "resnet50", "efficientnet"].map((m) => (
                  <button
                    key={m}
                    onClick={() => setModel(m)}
                    className={`w-full flex flex-col items-start rounded-2xl border p-4 transition-all ${
                      model === m ? "border-cyan-500 bg-cyan-50/50 shadow-sm" : "border-gray-100 bg-white hover:border-gray-200"
                    }`}
                  >
                    <span className={`font-bold capitalize ${model === m ? "text-cyan-700" : "text-gray-700"}`}>{m}</span>
                    <span className="text-[10px] text-gray-400 uppercase font-medium">Research Node</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-8">
            <div className="h-full flex flex-col rounded-3xl border border-white bg-white/60 p-8 shadow-2xl backdrop-blur-xl md:p-12 relative overflow-hidden">
                ...
                <AnimatePresence mode="wait">
                  {status === "queued" || status === "processing" ? (
                    ...
                    <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center text-center p-6">
                      ...
                      <p className="mt-1 text-xs text-gray-400 font-bold tracking-widest uppercase">Max 50MB (Video) | 10MB (Image)</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.label>
              ...
              <div className="flex gap-4">
                <button
                  disabled={!file || status === "queued" || status === "processing" || isOutOfCredits}
                  onClick={runInference}
                  className={`flex-1 rounded-2xl py-5 text-sm font-bold tracking-[0.2em] uppercase transition-all shadow-lg ${
                    !file || status === "queued" || status === "processing" || isOutOfCredits
                    ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                    : "bg-[#0A0A0A] text-white hover:bg-black active:scale-95 shadow-cyan-500/10"
                  }`}
                >
                  {isOutOfCredits ? "Insufficient Credits" : (status === "queued" || status === "processing") ? "Analyzing..." : "Begin Inference"}
                </button>
                {file && status === "idle" && (
